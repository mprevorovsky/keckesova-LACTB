...
* OVCAR8 samples were accidentally misnamed during preparation. Sample_ID column in 'sampleInfo' file is corrected accordingly for further processing and reflect real biological meaning of samples. Filenames are left intact.
...
* All samples show expected trends in LACTB transcript levels. But LACTB expression upon induction is MUCH higher in PEO4 cells than in OVCAR8 and Primary cells, but LACTB is mostly regulated at post-transcriptional level, so a western blot will be needed to compare LACTB protein levels in the respective sample groups.
...
dds$cell_line <- droplevels(dds$cell_line) # remove factor levels without samples
...
rm(res, res.sig, dds)
```
* There are 2759 DEG in Primary, 1603 in PEO4 and only 62 in OVCAR8.
...
write.csv(PEO4_DEG_up, "./DEG_RNA-seq/PEO4__upregulated.csv")
PEO4_DEG_down <- PEO4_DEG[PEO4_DEG$log2FoldChange < 0,] #787
write.csv(PEO4_DEG_down, "./DEG_RNA-seq/PEO4__downregulated.csv")
...
write.csv(PEO4_Primary_antipods_up, "./DEG_RNA-seq/PEO4_Primary_antipods_upregulated.csv")
PEO4_Primary_antipods_down <- PEO4_Primary_antipods[PEO4_Primary_antipods$PEO4_log2fold < 0,] #185
write.csv(PEO4_Primary_antipods_down, "./DEG_RNA-seq/PEO4_Primary_antipods_downregulated.csv")
...
write.csv(PEO4_DEG_exclusive_up, "./DEG_RNA-seq/PEO4_DEG_exclusive_upregulated.csv")
PEO4_DEG_exclusive_down <- PEO4_DEG_exclusive[PEO4_DEG_exclusive$log2FoldChange < 0,] #602
write.csv(PEO4_DEG_exclusive_down, "./DEG_RNA-seq/PEO4_DEG_exclusive_downregulated.csv")

#Removing DEGs with less than 1.5 fold change
PEO4_DEG_threshold_up <- PEO4_DEG_up[PEO4_DEG_up$log2FoldChange >= log2(1.5),] #439
write.csv(PEO4_DEG_threshold_up, "./DEG_RNA-seq/PEO4_DEG__upregulated_threshold.csv")
PEO4_DEG_threshlod_down <- PEO4_DEG_down[PEO4_DEG_down$log2FoldChange <= log2(1.5),] #785
write.csv(PEO4_DEG_threshlod_down, "./DEG_RNA-seq/PEO4_DEG__downregulated_threshold.csv")
...
* There are 381 intersections of PEO4 and Primary, 65 of them are regulated in the same direction.
* There are 16 intersections of OVCAR8 and Primary, 6 of them are regulated in the same direction.
* There are 13 intersections of PEO4 and OVCAR8, 3 of them are regulated in the same direction (this line is a subject to change after PEOvsPrimary and OVCARvsPrimary overlaps will be removed)
