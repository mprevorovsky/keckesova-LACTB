...
res <- cbind(counts[, 'gene_name'], res)
colnames(res)[1] <- 'gene_name'
write.csv(res, file = paste0(deg_dir, 'DESeq2results_OVCAR8_NOT0DOX_expressing_vs_OVCAR8_non-expressing.csv'), quote = FALSE)
res.sig <- res[which(res$padj <= padj_threshold), ]
res.sig <- res.sig[order(res.sig$log2FoldChange), ]
write.csv(res.sig, file = paste0(deg_dir, 'DESeq2results_OVCAR8_NOT0DOX_expressing_vs_OVCAR8_non-expressing.SIG.csv'), quote = FALSE)
rm(res, res.sig, dds)
...
OVCAR8_NOT0DOX_DEG <- read.csv("./DEG_RNA-seq/DESeq2results_OVCAR8_NOT0DOX_expressing_vs_OVCAR8_non-expressing.SIG.csv", header = TRUE, row.names = 1)
rownames(OVCAR8_NOT0DOX_DEG) <- substr(rownames(OVCAR8_NOT0DOX_DEG),6,20)
...
#OVCAR8_NOT0DOX_vs_Primary
OVCAR8_NOT0DOX_in_Primary <- OVCAR8_NOT0DOX_DEG[which(rownames(OVCAR8_NOT0DOX_DEG)%in%rownames(Primary_DEG)),] #1010 genes total
OVCAR8_NOT0DOX_in_Primary <- OVCAR8_NOT0DOX_in_Primary[order(OVCAR8_NOT0DOX_in_Primary$gene_name),]
Primary_in_OVCAR8_NOT0DOX <- Primary_DEG[which(rownames(Primary_DEG)%in%rownames(OVCAR8_NOT0DOX_DEG)),]
Primary_in_OVCAR8_NOT0DOX <- Primary_in_OVCAR8_NOT0DOX[order(Primary_in_OVCAR8_NOT0DOX$gene_name),]

dif_ways <- (OVCAR8_NOT0DOX_in_Primary$log2FoldChange > 0) == (Primary_in_OVCAR8_NOT0DOX$log2FoldChange > 0) #detect if expression changes in the same way
dif_fold <- abs(OVCAR8_NOT0DOX_in_Primary$log2FoldChange - Primary_in_OVCAR8_NOT0DOX$log2FoldChange) #calculate the fold difference in expression change

OVCAR8_NOT0DOX_vs_Primary <- data.frame(gene_name = OVCAR8_NOT0DOX_in_Primary$gene_name, 
                              OVCAR8_NOT0DOX_log2fold = OVCAR8_NOT0DOX_in_Primary$log2FoldChange, 
                              Primary_log2fold = Primary_in_OVCAR8_NOT0DOX$log2FoldChange, 
                              dif = dif_fold, 
                              same_way = dif_ways,
                              row.names = rownames(OVCAR8_NOT0DOX_in_Primary))
OVCAR8_NOT0DOX_vs_Primary <- OVCAR8_NOT0DOX_vs_Primary[order(OVCAR8_NOT0DOX_vs_Primary$dif),]

OVCAR8_NOT0DOX_vs_Primary_same_way <- OVCAR8_NOT0DOX_vs_Primary[OVCAR8_NOT0DOX_vs_Primary$same_way,] #786 of them
OVCAR8_NOT0DOX_DEG <- OVCAR8_NOT0DOX_DEG[!rownames(OVCAR8_NOT0DOX_DEG)%in%rownames(OVCAR8_NOT0DOX_vs_Primary_same_way),] #3766 left
write.csv(OVCAR8_NOT0DOX_DEG, "./DEG_RNA-seq/OVCAR_NOT0DOX_DEG.csv")
